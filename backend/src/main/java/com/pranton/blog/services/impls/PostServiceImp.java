...
public class PostServiceImp implements PostService {
    ...
    public List<Post> getPosts(UUID categoryId, UUID tagId) {
        ...
        if(categoryId != null && tagId != null) {
            ...
            return postRepository.findAllByStatusAndCategoryAndTagsContaining(PostStatus.PUBLISHED, categoryById, tagById);
        }

        if(categoryId != null) {
            Category categoryById = categoryService.getCategoryById(categoryId);
            return postRepository.findAllByStatusAndCategory(PostStatus.PUBLISHED, categoryById);
        }
        if(tagId != null) {
            Tag tagById = tagService.getTagById(tagId);    
            return postRepository.findAllByStatusAndTagsContaining(PostStatus.PUBLISHED, tagById);        
        }
    ...
    @Override
    public List<Post> getPostByAuthor(User user, PostStatus status) {
        // TODO Auto-generated method stub
        return postRepository.findAllByAuthorAndStatus(user, status);
    }
